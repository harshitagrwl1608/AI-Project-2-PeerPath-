...
const DiscoveryFeed = () => {
    ...
    const filteredUsers = (users || []).filter(u => {
        ...
        const matchesSearch = matchesSkill || matchesName;

        if (!matchesSearch) return false;

        if (feedMode === 'personalized' && userProfile) {
            const theyTeachWhatILearn = userProfile.skillsLearn?.some(myLearnSkill => 
                u.skillsTeach?.some(theirTeachSkill => theirTeachSkill.toLowerCase() === myLearnSkill.toLowerCase())
            );

            const theyLearnWhatITeach = userProfile.skillsTeach?.some(myTeachSkill =>
                u.skillsLearn?.some(theirLearnSkill => theirLearnSkill.toLowerCase() === myTeachSkill.toLowerCase())
            );
                ...
                <div className="flex space-x-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    <div className="flex bg-gray-100 p-1 rounded-xl flex-shrink-0">
                        <button 
                            ...
                            className={`px-4 py-2 text-sm font-medium rounded-lg transition ${feedMode === 'personalized' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            For You
                        </button>
                        <button 
                            onClick={() => setFeedMode('all')}
                            className={`px-4 py-2 text-sm font-medium rounded-lg transition ${feedMode === 'all' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            All Peers
                        </button>
                    </div>
